<x-dashboard-layout>
    <div class="p-6 md:p-8 space-y-8 max-w-7xl mx-auto w-full">
        ...
        <header class="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-4">
            <div>
                <h2 class="font-headline text-xl font-extrabold text-on-surface">Prediksi Stok (SMA)</h2>
                <p class="text-xs text-slate-500 font-medium">Simple Moving Average (SMA) untuk Optimasi Stok</p>
            </div>
            <div class="flex gap-3">
                <div class="flex items-center gap-2 bg-surface-container-lowest border border-outline-variant px-4 py-2 rounded-lg text-primary text-xs font-bold uppercase tracking-widest">
                    <span class="text-xs font-bold text-on-surface-variant uppercase tracking-widest">Status:</span>
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
            ...
            <div class="bg-surface-container-lowest p-6 rounded-xl relative overflow-hidden group">
                <div class="flex flex-col">
                    <span class="text-[11px] font-headline font-extrabold uppercase tracking-[0.2em] text-slate-500 mb-2">Items Aman</span>
            ...
            <div class="bg-surface-container-lowest p-6 rounded-xl relative overflow-hidden group">
                <div class="flex flex-col">
                    <span class="text-[11px] font-headline font-extrabold uppercase tracking-[0.2em] text-slate-500 mb-2">Peringatan</span>
                    <div class="flex items-baseline gap-2">
        ...
        <form method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div class="p-5 bg-surface-container-low rounded-2xl flex flex-col gap-2 justify-end">
                <button type="submit" class="bg-primary text-white px-6 py-2 rounded-lg text-xs font-bold uppercase tracking-widest hover:bg-primary/90 transition-all flex items-center gap-2 justify-center w-full">
                    <span class="material-symbols-outlined text-sm">filter_list</span>
                    Filter
                </button>
@if(request('status'))
                    <a href="{{ route('manager.prediction') }}" class="bg-error text-white px-6 py-2 rounded-lg text-xs font-bold uppercase tracking-widest hover:bg-error/90 transition-all flex items-center gap-2 justify-center w-full">
                        Clear
                    </a>
                    @endif
            </div>
            ...
            <table class="w-full text-left">
                ...
                <tbody class="divide-y divide-outline-variant/10 text-sm">
                    @forelse($materialsWithPredictions as $material)
                        <tr class="hover:bg-surface-container-low transition-colors {{ $material->stock_status == 0 || $material->stock_status == 1 ? 'bg-error/5' : '' }}">
                            <td class="px-6 py-4">
                                <div class="flex flex-col">
                            ...
                            <td class="px-6 py-4 text-right font-medium {{ $material->current_stock < $material->minimum_stock ? 'text-error' : '' }}">
                            ...
                            <td class="px-6 py-4">
                                @if($material->days_remaining !== null)
                                    <span class="font-bold {{ $material->days_remaining <= 7 ? 'text-error' : ($material->days_remaining <= 14 ? 'text-tertiary' : 'text-primary') }}">
